...
def handle_player(client_socket, isCross, message):
    global crossMove, field, isStarted
    while not(isStarted):
        continue
    if(len(message) > 7):
        isCross = int(message[6])
        field[int(message[7])] = int(isCross)+1
...
def backup_client(client_socket):
    global field,isStarted,crossMove, isMain
...
def run_server():
    ...
    while True:
        ...
        if('client' in message):
            ...
            #if(isMain):
            client_thread = threading.Thread(target=handle_player, args=(client_socket, not(nextPlayerIsCross), message))
            #else:
            #    client_thread = threading.Thread(target=handle_player, args=(client_socket, False))
            client_thread.start()
            nextPlayerIsCross = not(nextPlayerIsCross)
            if(isMain):
